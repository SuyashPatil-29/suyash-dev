...
const components = {
  ...
  h2: ({ className, ...props }) => (
    <h2
      className={cn(
        "mt-10 scroll-m-20 text-3xl font-semibold tracking-tight first:mt-0 text-rose-500",
        className
      )}
      {...props}
    />
  ...
  p: ({ className, ...props }) => (
    <p
      className={cn("leading-7 [&:not(:first-child)]:mt-6 text-white", className)}
      {...props}
    />
  ),
  ul: ({ className, ...props }) => (
  ...
  li: ({ className, ...props }) => (
    <li className={cn("mt-2 text-gray-300", className)} {...props} />
  ),
